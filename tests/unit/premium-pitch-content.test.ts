...
describe('resolvePremiumStage', () => {
  it('leads with friends when the rider has any', () => {
    expect(resolvePremiumStage({ favCount: 1, reminderCount: 0, friendCount: 2 }))
      .toBe('has_friends')
  })
  ...
  it('prefers the friends angle over the alerts angle', () => {
    expect(resolvePremiumStage({ favCount: 1, reminderCount: 9, friendCount: 1 }))
      .toBe('has_friends')
  })

  it('falls back to the digest angle for a rider who only sets alerts', () => {
    expect(resolvePremiumStage({ favCount: 1, reminderCount: 3, friendCount: 0 }))
      .toBe('active_alerts')
  })

  it('pitches the spot limit when there is nothing to infer', () => {
    expect(resolvePremiumStage({ favCount: 0, reminderCount: 0, friendCount: 0 }))
      .toBe('default')
  })

  it('does not trust negative or junk counts', () => {
    expect(resolvePremiumStage({ favCount: -2, reminderCount: -1, friendCount: -5 }))
      .toBe('default')
    expect(resolvePremiumStage({
      favCount: NaN as any, reminderCount: NaN as any, friendCount: NaN as any,
    })).toBe('default')
  })
})
...
describe('buildPremiumHookHtml', () => {
  const STATES = [
    { favCount: 0, reminderCount: 0, friendCount: 0 },
    { favCount: 1, reminderCount: 2, friendCount: 0 },
    { favCount: 3, reminderCount: 2, friendCount: 4 },
  ]
  ...
  it('counts friends in words a human would use', () => {
    const one  = buildPremiumHookHtml({ favCount: 1, reminderCount: 0, friendCount: 1 }, LINKS, 'Sam')
    const many = buildPremiumHookHtml({ favCount: 1, reminderCount: 0, friendCount: 4 }, LINKS, 'Sam')
    expect(one).toContain('a rider')
    expect(many).toContain('4 riders')
  })

  it('counts alerts the same way', () => {
    const one  = buildPremiumHookHtml({ favCount: 1, reminderCount: 1, friendCount: 0 }, LINKS, 'Sam')
    const many = buildPremiumHookHtml({ favCount: 1, reminderCount: 3, friendCount: 0 }, LINKS, 'Sam')
